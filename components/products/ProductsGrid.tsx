...
const ProductsGrid = ({ products }: ProductsGridProp) => {
    return (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-2.5 lg:grid-cols-3 lg:gap-5">
            {products?.map((product) => {
                const productId = product.id;
                const price = formatCurrency(product.price);

                return (
                    <article key={productId} className="relative bg-muted border rounded-lg overflow-hidden group transform hover:shadow-xl transition-shadow duration-500 py-5 md:py-0">
